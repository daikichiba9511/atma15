...
def train_one_fold(
    ...
    train_dmatrix = xgb.DMatrix(
        train_data[features], label=train_data["score"].to_numpy().ravel()
    )
    valid_dmatrix = xgb.DMatrix(
        valid_data[features], label=valid_data["score"].to_numpy().ravel()
    )
    ...
    model_xgb = xgb.train(
        ...
        #         rounds=100, metric_name="rmse", data_name="valid"
        #     )
        # ],
    )
...
def predict(
    ...
    return model.predict(test_dmatrix, iteration_range=(0, model.best_iteration + 1))
...
def main() -> None:
    ...
    trained_preprocess_models: TrainedModels = {
    ...
    test_merged = test_preprocess(
        test_merged,
        trained_preprocess_models,
        all_merged=all_df,
    )

    logger.info(f"{train_merged.shape = }")
    logger.info(f"{test_merged.shape = }")
    logger.info(f"{train_merged.head(10) = }")
    logger.info(f"{test_merged.head(10) = }")

    del trained_preprocess_models
    gc.collect()
